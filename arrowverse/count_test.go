...
func TestEpisodeNumbers(t *testing.T) {
	...
	showSeasonEpisodeNumbers := map[string]map[int]int{
	...
	is.NoErr(errGELU)
	is.Equal(len(episodeListURLs), len(showSeasonEpisodeNumbers))
	...
	for s, elu := range episodeListURLs {
		...
		t.Run(s, func(t *testing.T) {
			...
			seasonNumbers, haveShowNumbers := showSeasonEpisodeNumbers[s]
